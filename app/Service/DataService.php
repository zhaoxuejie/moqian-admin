            ...
            case 'menus':
                switch ($kind[1]){
                    case 'add_or_update':
                        ...
                        $roles = $inputs['roles'];
                        if($inputs['id']){
                            if (is_config_id($inputs['id'], "admin.menu_table_cannot_manage_ids", false))return ['status'=>0,'msg'=>trans('cfg.menus.notedit')];
                            $model->exists = true;
                            $model->id = $inputs['id'];
                        }
                        ...
                        try{
                            if (!$model->save()) {
                                return ['status'=>0,'msg'=>trans('cfg.common.fail')];
                            }
                        ...
                        return ['status'=>1,'msg'=>trans('cfg.common.success')];
                        break;

                    case 'delete':
                        $model->id = $inputs['id'];
                        $model->exists = true;
                        if($model->delete())return ['status'=>1,'msg'=>trans('cfg.common.success')];
                        else return ['status'=>0,'msg'=>trans('cfg.common.fail')];
                        break;
                    default:
                        return ['status'=>0,'msg'=>trans('cfg.common.wrong')];
                }
                break;
            case 'users':
                switch ($kind[1]){
                    case 'add_or_update':
                        ...
                        if($inputs['id']){
                            if(is_config_id($inputs['id'], "admin.user_table_cannot_manage_ids", false))return ['status'=>0,'msg'=>trans('cfg.users.notedit')];
                            $model->exists = true;
                            $model->id = $inputs['id'];
                        }
                        try{
                    ...
                    case 'update_pwd':
                        ...
                        if(!App::make('hash')->check($inputs['oldpwd'],$userinfo['password']))return ['status'=>0,'msg'=>trans('cfg.users.pwd_false')];
            ...
            case 'roles':
                switch ($kind[1]){
                    case 'add_or_update':
                        ...
                        if($inputs['id']){
                            if(is_config_id($inputs['id'], "admin.role_table_cannot_manage_ids", false))return ['status'=>0,'msg'=>trans('cfg.roles.notedit')];
                            $model->exists = true;
                            $model->id = $inputs['id'];
                        }
                        try{
                    ...
                    case 'delete':
                        if($model->isAbleDel($inputs['id']))return ['status'=>0,'msg'=>trans('cfg.roles.have_user')];
                        $model->id = $inputs['id'];
                        $model->exists = true;
                        if($model->delete())return ['status'=>1,'msg'=>trans('cfg.common.success')];
                        else return ['status'=>0,'msg'=>trans('cfg.common.fail')];
            ...
            case 'permissions':
                switch ($kind[1]){
                    case 'add_or_update':
                        ...
                        if($inputs['id']){
                            if (is_config_id($inputs['id'], "admin.permission_table_cannot_manage_ids", false))return ['status'=>0,'msg'=>trans('cfg.menus.notedit')];
                            $model->exists = true;
                            $model->id = $inputs['id'];
                        }
                        try{
